 ...
 * @author peter
 */
public class ISUSDataCreationForm extends MemoryWindow implements DataProcessor
{

    private static Logger logger = Logger.getLogger(ISUSDataCreationForm.class.getName());
    ...
    private void runButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_runButtonActionPerformed
        ...
        if(deleteDataBox.isSelected())
        {
            ...
            RawInstrumentData.deleteDataForMooringAndParameter(selectedMooring.getMooringID(), "NTRI");
        }
    ...
    {
        // Create the averaged nitrate value        
        
        String SQL = "INSERT INTO raw_instrument_data SELECT min(source_file_id) AS source_file_id, " +
                                                                                " min(instrument_id) as instrument_id, " +
                                                                                " " + StringUtilities.quoteString(selectedMooring.getMooringID()) + " as mooring_id, " +
                                                                                " min(data_timestamp) as data_timestamp, " +
                                                                                " min(latitude) as latitude, " +
                                                                                " min(longitude) as longitude, " + 
                                                                                " min(depth) as depth, " +
                                                                                " 'NTRI' as parameter_code, " +
                                                                                " avg(parameter_value) as parameter_value, " +
                                                                                " 'AVG' as quality_code " +
                                                                                " FROM raw_instrument_data WHERE mooring_id = " + StringUtilities.quoteString(selectedMooring.getMooringID()) + " " +
                                                                                                            " AND parameter_code = 'NTRI_RAW' " +
                                                                                                            " GROUP BY date_trunc('hour', data_timestamp) ORDER BY data_timestamp ";
