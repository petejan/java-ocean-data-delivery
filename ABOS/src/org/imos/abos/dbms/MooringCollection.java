...
public class MooringCollection extends AbstractTableModel
        implements EditableTableInterface, ExtendedTableMetaData
{
    private static Logger logger = Logger.getLogger(MooringCollection.class.getName());

    private javax.swing.JFrame parentFrame = null;
    protected ArrayList<Mooring> rows = null;

    // save the original of the focused row both to compare values and to undo edits
    private Mooring savedRow = null;

    public void loadFromPersistentStore()
    {
        rows = Mooring.selectAll();
    }
    ...
    {
        return Mooring.getColumnName( column );
    }
    ...
    {
        return Mooring.getColumnClass( column);
    }

    public int getColumnCount()
    {
        return Mooring.getColumnCount();
    }
    ...
    public boolean saveEditedRow(int row)
    {
        try
        {
            Mooring editingRow = rows.get( row );
    ...
    public void storeRowBeforeEditing(int row)
    {
        try
        {
            savedRow =  (Mooring) ((Mooring)rows.get(row)).clone();
        }
    ...
    public boolean isCellReallyEditable(int row, int column)
    {
        ...
        Mooring focusedRow = rows.get(row);
    ...
    {
        return Mooring.getColumnWidths();
    }
