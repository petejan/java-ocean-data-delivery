...
public class InstrumentDataParserCollection extends AbstractTableModel
        implements EditableTableInterface, ExtendedTableMetaData
{
    private static Logger logger = Logger.getLogger(InstrumentDataParserCollection.class.getName());

    private javax.swing.JFrame parentFrame = null;
    protected ArrayList<InstrumentDataParser> rows = null;

    // save the original of the focused row both to compare values and to undo edits
    private InstrumentDataParser savedRow = null;

    public void loadFromPersistentStore()
    {
        rows = InstrumentDataParser.selectAll();
    }
    ...
    {
        return InstrumentDataParser.getColumnName( column );
    }
    ...
    {
        return InstrumentDataParser.getColumnClass( column);
    }

    public int getColumnCount()
    {
        return InstrumentDataParser.getColumnCount();
    }
    ...
    {
        InstrumentDataParser row = rows.get(aRow);
    ...
    {
        InstrumentDataParser newRow = new InstrumentDataParser();
        newRow.setNew(true);
    ...
    public boolean saveEditedRow(int row)
    {
        try
        {
            InstrumentDataParser editingRow = rows.get( row );
    ...
    public void storeRowBeforeEditing(int row)
    {
        try
        {
            savedRow =  (InstrumentDataParser) ((InstrumentDataParser)rows.get(row)).clone();
        }
    ...
    public boolean isCellReallyEditable(int row, int column)
    {
        ...
        InstrumentDataParser focusedRow = rows.get(row);
    ...
    {
        return InstrumentDataParser.getColumnWidths();
    }
