...
public class WriteMRU_SOFS_1
{

    static Date dataTs[] = new Date[10000];
    static int n;

    public void readDataFile(String file) throws IOException
    {
        String thisLine;
        StringTokenizer st;
        String token;
        ...
        while ((thisLine = br.readLine()) != null)
        {
            ...
            while (st.hasMoreTokens())
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            ts = sdf.parse(token);
                            // System.out.println(n + " " + sdf.format(ts));
                        }
                    }
                }
            ...
            {
                dataTs[n] = ts;
                // System.out.println("n " + n + " " + sdf.format(ts));
            }
        }
    ...
    public static void main(String args[]) throws Exception
    {
        ...
        WriteMRU_SOFS_1 wMRU = new WriteMRU_SOFS_1();

        wMRU.readDataFile("data/SOFS-1/SOFS1_SD.csv");

        // Create the file.
        File f = new File(args[0]);
        String filename = "SOFS-1-" + f.getName() + ".nc";
        ...
        try
        {
            ...
            Date dataStart = null, dataEnd = null;
            ...
            for (int fileNo = 0; fileNo < args.length; fileNo++)
            {
                ...
                dataTime.setInt(fileNo, (int) ((dataTs[fNo].getTime() - tz) / 1000));
                if (dataStart == null)
                {
                    dataStart = dataTs[fNo];
                }
                dataEnd = dataTs[fNo];
                ...
                double waveHeight = wd.calculate(WaveCalculator.DF, logSpec);
                dataSWH.setFloat(fileNo, (float) waveHeight);

                System.out.println("FILE " + args[fileNo] + " time " + dataTs[fNo] + " wave height " + waveHeight + " load av " + loadStats.getMean());
            }
