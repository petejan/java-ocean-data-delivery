...
for path_file in sys.argv[1:len(sys.argv)]:
	...
	dt_time = [dt.datetime(1950, 1, 1) + dt.timedelta(days=t) for t in time]
	...
	for nc_attr in nc_attrs:
		...
		text += nc_attr + '\t:' + repr(nc.getncattr(nc_attr)) + '\n'
	...
	for plot in to_plot:
		...
		text = "Variable : " + plot_var.name + " dims : " + repr(plot_var.dimensions) + "\n"
		nc_attrs = plot_var.ncattrs()
		# print "NetCDF Global Attributes:"
		for nc_attr in nc_attrs:
			# print '\t%s:' % nc_attr, repr(nc.getncattr(nc_attr))
			text += nc_attr + '\t:' + repr(plot_var.getncattr(nc_attr)) + '\n'
		...
		if hasattr(plot_var, 'ancillary_variables'):
			qc_var = plot_var.getncattr('ancillary_variables')

			qc = nc.variables[qc_var][:]
