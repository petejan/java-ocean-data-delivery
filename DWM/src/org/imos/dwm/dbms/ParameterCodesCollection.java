...
public class ParameterCodesCollection extends AbstractTableModel
        implements EditableTableInterface, ExtendedTableMetaData
{
    private static Logger logger = Logger.getLogger(ParameterCodesCollection.class.getName());

    private javax.swing.JFrame parentFrame = null;
    protected ArrayList<ParameterCodes> rows = null;

    // save the original of the focused row both to compare values and to undo edits
    private ParameterCodes savedRow = null;

    public void loadFromPersistentStore()
    {
        rows = ParameterCodes.selectAll();
    }
    ...
    {
        return ParameterCodes.getColumnName( column );
    }
    ...
    {
        return ParameterCodes.getColumnClass( column);
    }

    public int getColumnCount()
    {
        return ParameterCodes.getColumnCount();
    }
    ...
    {
        ParameterCodes row = rows.get(aRow);
    ...
    {
        ParameterCodes newRow = new ParameterCodes();
        newRow.setNew(true);
    ...
    public boolean saveEditedRow(int row)
    {
        try
        {
            ParameterCodes editingRow = rows.get( row );
    ...
    public void storeRowBeforeEditing(int row)
    {
        try
        {
            savedRow =  (ParameterCodes) ((ParameterCodes)rows.get(row)).clone();
        }
    ...
    public boolean isCellReallyEditable(int row, int column)
    {
        ...
        ParameterCodes focusedRow = rows.get(row);
    ...
    {
        return ParameterCodes.getColumnWidths();
    }
